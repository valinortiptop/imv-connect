...
export default function Sales() {
  ...
  const { data: salesStats = emptySalesStats, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return {
        ...
        byClient: normalizeRows(data.byClient, [
          "orders",
          "revenue",
          "profit",
          "profitBonif",
        ...
        byProduct: normalizeRows(data.byProduct, [
          "revenue",
          "profit",
          "profitBonif",
          "marginPct",
        ...
        byBrand: normalizeRows(data.byBrand, [
          "revenue",
          "profit",
          "profitBonif",
          "marginPct",
        ...
        byOrder: normalizeRows(data.byOrder, [
          "revenue",
          "profit",
          "profitBonif",
          "marginPct",
  ...
  const { kpis, pendingKpis, prevKpis, dailyTrend, byClient, byProduct, byBrand, byOrder } =
    salesStats;

  // Comparison deltas
  const deltaRevenue =
    prevKpis.totalRevenue > 0
          ...
          <GlowCard>
            <div className="p-5">
              ...
              <p className="text-2xl font-bold text-foreground">
                {mxnFmt.format(kpis.totalRevenue)}
              </p>
              <p className="text-xs text-muted-foreground mt-1">
                {kpis.uniqueOrders} pedidos · Ticket prom. {mxnFmt.format(kpis.avgTicket)}
              </p>
              {dateFrom === getFirstOfMonth() && deltaRevenue !== 0 && (
                <p
                  className={cn(
                    "text-xs font-medium mt-1",
                    deltaRevenue > 0 ? "text-green-400" : "text-red-400",
                  )}
                >
                  {deltaRevenue > 0 ? "+" : ""}
                  {deltaRevenue.toFixed(1)}% vs mes anterior
                </p>
              )}
            </div>
          </GlowCard>
          ...
          <GlowCard>
            <div className="p-5">
              ...
              <div className="space-y-1">
                <div className="flex items-baseline gap-2">
                  <p className="text-2xl font-bold text-foreground">
                    {mxnFmt.format(kpis.realizedProfitBonif)}
                  </p>
                  <span className="text-xs text-muted-foreground">realizada</span>
                </div>
                <div className="flex items-baseline gap-2">
                  <p className="text-lg font-semibold text-muted-foreground">
                    {mxnFmt.format(kpis.impliedProfitBonif)}
                  </p>
                  <span className="text-xs text-muted-foreground">implicada</span>
                </div>
              </div>
              ...
              <div className="space-y-1">
                <div className="flex items-baseline gap-2">
                  ...
                  <span className="text-xs text-muted-foreground">
                    {pendingKpis.orders === 1 ? "pedido" : "pedidos"}
                  </span>
                </div>
                <div className="flex items-baseline gap-2">
                  <p className="text-lg font-semibold text-muted-foreground">
                    {mxnFmt.format(pendingKpis.revenue)}
                  </p>
                  <span className="text-xs text-muted-foreground">valor</span>
                </div>
                <div className="flex items-baseline gap-2">
                  <p className="text-lg font-semibold text-green-400">
                    {mxnFmt.format(pendingKpis.profit)}
                  </p>
                ...
                {dailyTrend.length > 0 &&
                  (() => {
                    ...
                    const yTicks = Array.from(
                      { length: Math.ceil(yMax / step) + 1 },
                      (_, i) => i * step,
                    );
                    const points = dailyTrend.map((d, i) => ({
                      x:
                        padL +
                        (dailyTrend.length > 1 ? (i / (dailyTrend.length - 1)) * plotW : plotW / 2),
                        ...
                        <div className="p-5">
                          ...
                          <div className="w-full relative" data-chart-wrap="1">
                            ...
                            >
                              ...
                              <g clipPath="url(#line-reveal)">
                                ...
                                {points.map((p, i) => {
                                  const delay =
                                    points.length > 1 ? (i / (points.length - 1)) * 0.8 : 0;
                                  return (
                                    <g key={p.date}>
                                      <circle
                                        cx={p.x}
                                      ...
                                      {(dailyTrend.length <= 15 ||
                                        i % Math.ceil(dailyTrend.length / 12) === 0) && (
                                        <text
                                          x={p.x}
                                          y={chartH - 6}
                                          textAnchor="middle"
                              ...
                              {points.map((p, i) => {
                                const colW =
                                  points.length > 1 ? plotW / (points.length - 1) : plotW;
                                ...
                                >
                                  <div className="bg-popover border rounded-lg shadow-lg px-5 py-3 whitespace-nowrap">
                                    <p className="font-semibold text-base">
                                      {format(parseLocalDate(p.date), "dd MMM yyyy", {
                                        locale: es,
                                      })}
                                    </p>
                          ...
                          <div className="flex items-center justify-between mt-2">
                            <div className="flex items-center gap-1.5 text-xs">
                              ...
                              <span className="text-muted-foreground">Ventas diarias</span>
                            </div>
                            <span className="text-xs text-muted-foreground">
                              ...
                              <div className="flex items-center justify-between mb-1">
                                ...
                                <div className="text-right shrink-0 ml-2">
                                  <span className="text-sm font-medium text-foreground">
                                    {mxnFmt.format(c.revenue)}
                                  </span>
                                  <span className="text-xs text-green-400 ml-2">
                                    +{mxnFmt.format(c.profit)}
                                  </span>
                                </div>
                              </div>
                                  ...
                                  {byBrand.map((b, i) => {
                                    ...
                                    return (
                                      <div key={b.name} className="flex items-center gap-2.5">
                                        ...
                                        <span className="text-sm text-muted-foreground whitespace-nowrap ml-auto tabular-nums">
                                          {pct.toFixed(0)}%
                                        </span>
                                        <span className="text-sm font-medium text-foreground whitespace-nowrap tabular-nums">
                                          {mxnFmt.format(b.revenue)}
                                        </span>
                                      </div>
                                    );
                                  })}
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="border-border">
                      ...
                      <TableHead className="text-muted-foreground text-center w-[80px]">
                        Pedidos
                      </TableHead>
                      <TableHead className="text-muted-foreground text-center w-[80px]">
                        Bultos
                      </TableHead>
                      <TableHead className="text-muted-foreground text-right w-[120px]">
                        Ventas
                      </TableHead>
                      <TableHead className="text-muted-foreground text-right w-[120px]">
                        Utilidad
                    ...
                    {isLoading ? (
                      ...
                      <>
                        <TableRow className="border-border bg-muted/30 font-semibold">
                          ...
                          <TableCell className="text-right tabular-nums text-sm text-cyan-400">
                            {mxnFmt.format(kpis.realizedProfitBonif)}
                          </TableCell>
                          <TableCell className="text-center">
                            <Badge variant="secondary" className="text-xs">
                              {pctFmt(kpis.marginPct)}
                            </Badge>
                          </TableCell>
                          <TableCell className="text-center">
                            <Badge variant="secondary" className="text-xs">
                              {pctFmt(kpis.marginBonifPct)}
                            </Badge>
                          </TableCell>
                        </TableRow>
                        {byClient.map((c) => (
                          <TableRow key={c.id} className="border-border">
                            ...
                            <TableCell className="text-right tabular-nums text-sm font-medium text-green-400">
                              {mxnFmt.format(c.profit)}
                            </TableCell>
                            <TableCell className="text-right tabular-nums text-sm font-medium text-cyan-400">
                              {mxnFmt.format(c.profitBonif)}
                            </TableCell>
                            <TableCell className="text-center">
                              <Badge
                                variant="secondary"
                ...
                <Table>
                  ...
                  <TableBody>
                    {isLoading ? (
                      ...
                      <>
                        <TableRow className="border-border bg-muted/30 font-semibold">
                          <TableCell colSpan={3} className="text-foreground text-sm">
                            Total
                          </TableCell>
                          <TableCell className="text-center tabular-nums text-sm">
                            {kpis.totalUnits}
                        ...
                        {byProduct.map((p) => (
                          <TableRow key={p.id} className="border-border">
                            <TableCell className="font-mono text-primary font-medium text-sm">
                              {p.clave}
                            </TableCell>
                      ...
                      <>
                        ...
                        {byBrand.map((b, i) => (
                          <TableRow key={b.name} className="border-border">
                            ...
                            <TableCell className="text-right tabular-nums text-sm font-medium text-green-400">
                              {mxnFmt.format(b.profit)}
                            </TableCell>
                            <TableCell className="text-right tabular-nums text-sm font-medium text-cyan-400">
                              {mxnFmt.format(b.profitBonif)}
                            </TableCell>
                            <TableCell className="text-center">
                              <Badge
                                variant="secondary"
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="border-border">
                      ...
                      <TableHead className="text-muted-foreground">Cliente</TableHead>
                      <TableHead className="text-muted-foreground text-center w-[70px]">
                        Líneas
                      </TableHead>
                      <TableHead className="text-muted-foreground text-center w-[80px]">
                        Bultos
                        ...
                        {byOrder.map((o) => (
                          <TableRow key={o.id} className="border-border">
                            ...
                            <TableCell className="text-sm text-muted-foreground whitespace-nowrap">
                              {format(parseLocalDate(o.date), "dd MMM yyyy", { locale: es })}
                            </TableCell>
                            <TableCell className="font-medium text-foreground text-sm">
                              {o.clientName}
                            </TableCell>
                            ...
                            <TableCell className="text-right tabular-nums text-sm font-medium">
                              {mxnFmt.format(o.revenue)}
                            </TableCell>
                            <TableCell className="text-right tabular-nums text-sm font-medium text-green-400">
                              {mxnFmt.format(o.profit)}
                            </TableCell>
                            <TableCell className="text-right tabular-nums text-sm font-medium text-cyan-400">
                              {mxnFmt.format(o.profitBonif)}
                            </TableCell>
                            <TableCell className="text-center">
                              <Badge
                                variant="secondary"
